...
<div class="mb-4" style="padding-left:30px; padding-right:30px; ">
    <div style="background-color: #ffffff; color:#434443; padding-left:15px; border-radius:3px; font-size:17px; padding-top:5px; padding-bottom:5px;"><i class="fas fa-solid fa-briefcase"></i> / Editar cliente</div>
</div>

<div class="" style="padding-left:30px; padding-right:30px; ">
    <div class="card-body" style="background-color: #FFFFFF; padding-left:55px; padding-right:55px;">

        <div class="pb-3">
            <a class="flecha-atras" style="font-size: 20px;" href="{{ route('admin.cliente.index') }}">
                <span>&#8592;</span>Regresar
            </a>
        </div>

        <form id="formCliente" action="{{ route('admin.cliente.update', $cliente->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="row">
                <div class="col-4">
                    <div class="form-group position-relative">
                        <label for="persona">Nombre de cliente:</label>
                        ...
                        <div class="input-group"> <!-- Grupo del input -->
                            ...
                            <input id="persona" name="persona" class="form-control @error('persona_id') is-invalid @enderror borde-input" type="text" placeholder="Buscar Persona..." autocomplete="off" value="{{ old('persona', ($cliente->persona->nombre ?? '') . ' ' . ($cliente->persona->apellido ?? '')) }}">
                            
                            <input type="hidden" name="persona_id" id="persona_id" value="{{ old('persona_id', $cliente->persona_id ?? '') }}"> <!-- Para almacenar el ID de la persona seleccionada -->
                        </div>
                
                        <div id="persona-lista" class="list-group position-absolute w-100" style="z-index: 100;"></div> <!-- Donde se mostrarán los resultados -->
                ...
                <div class="col-4">
                    <div class="form-group">
                        ...
                        <input class="form-control @error('razon_social') is-invalid @enderror focus-input borde-input" type="text" name="razon_social" value="{{ $cliente->razon_social }}" placeholder="razón social/ caso de no tener dejar vacio..." autocomplete="off">
                        @error('razon_social')
                                <p class="invalid-feedback" style="color: red; font-size: 0.8em;">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="col-4">
                    <div class="form-group">
                        ...
                        <input class="form-control @error('nit') is-invalid @enderror focus-input borde-input" type="text" name="nit" value="{{ $cliente->nit }}" placeholder="Ingrese NIT/ caso de no tener ingresar CI..." autocomplete="off">
                        @error('nit')
                                <p class="invalid-feedback" style="color: red; font-size: 0.8em;">{{ $message }}</p>
                        @enderror
                    </div>
...
@section('js')
    ...
    <script>
        ...
        <script>
            $(document).ready(function() {
                $('#formCliente').on('keydown', function(event) {
                    if (event.key === 'Enter') {
        ...
        <script>
            function confirmarCancelar() {
                if (confirm("¿Estás seguro de que deseas cancelar la edición?")) {
                    window.location.href = "{{ route('admin.cliente.index') }}";
                }
            }
        </script>
